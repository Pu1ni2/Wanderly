  ...
  if (!budgetUSD) {
    return (
      <div className="text-sm text-stone-600">
        Estimated cost:{" "}
        <span className="font-semibold text-stone-900">${spentUSD.toLocaleString()}</span>
      </div>
    );
  }
    ...
    <div className="w-full">
      <div className="flex items-baseline justify-between mb-1.5">
        ...
        <span className="text-sm tabular-nums">
          <span className={`font-semibold ${over ? "text-red-600" : "text-stone-900"}`}>
            ${spentUSD.toLocaleString()}
          </span>
          <span className="text-stone-500"> / ${budgetUSD.toLocaleString()}</span>
        </span>
      </div>
      <div className="h-2 w-full rounded-full bg-stone-50 overflow-hidden border" style={{ borderColor: "var(--border)" }}>
